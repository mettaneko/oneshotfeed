...
*25.12.8* - Исправление протухания ссылок (403 Forbidden) и добавление режима тех. работ.
    ...
    const isChannel = !!senderChatId || msg.chat.type === 'channel';
    ...
    if (text === '/start') {
      ...
      if (text.startsWith('/add') || text.includes('tiktok.com')) {
        ...
        if (mode === 'on') {
          await fetch(`${DB_URL}/set/maintenance_mode/true`, {
            headers: { Authorization: `Bearer ${DB_TOKEN}` }
          });
          await sendMessage(token, chatId, '🔴 <b>Заглушка ВКЛЮЧЕНА!</b>', null, 'HTML');
        } else if (mode === 'off') {
          await fetch(`${DB_URL}/set/maintenance_mode/false`, {
            headers: { Authorization: `Bearer ${DB_TOKEN}` }
          });
        ...
        try {
          const r = await fetch(`${DB_URL}/llen/feed_videos`, {
            headers: { Authorization: `Bearer ${DB_TOKEN}` }
          });
          const d = await r.json();
          await sendMessage(token, chatId, `📊 Видео: ${d.result || 0}`, null, 'HTML');
        } catch (e) {
          await sendMessage(token, chatId, '❌ Ошибка Redis');
        }
      } else if (text === '/status') {
        try {
          const r = await fetch(`${DB_URL}/ping`, {
            headers: { Authorization: `Bearer ${DB_TOKEN}` }
          });
          await sendMessage(token, chatId, `Redis: ${r.ok ? '🟢 OK' : '🔴 ERROR'}`, null, 'HTML');
        } catch (e) {
          await sendMessage(token, chatId, '❌ Нет коннекта');
        }
      } else if (text.startsWith('/broadcast')) {
        const bText = text.replace('/broadcast', '').trim();
        let users = [];
        try {
          const r = await fetch(`${DB_URL}/smembers/all_bot_users`, {
            headers: { Authorization: `Bearer ${DB_TOKEN}` }
          });
          const d = await r.json();
          users = d.result || [];
