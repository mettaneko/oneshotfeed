...
function validateTelegramAuth(initData, botToken) {
    ...
    } catch (e) { console.error('Auth validation error:', e); }
    return null;
}
...
export default async function handler(req, res) {
    ...
    try {
        ...
        if (type === 'following') {
            ...
            // --- ИСПРАВЛЕНО ЗДЕСЬ ---
            // Было: const userSubsKey = `user_subs:${user.id}`;
            const userSubsKey = `subs:${user.id}`; // Твой формат ключа
